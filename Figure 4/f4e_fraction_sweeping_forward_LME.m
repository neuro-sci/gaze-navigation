function f4e_fraction_sweeping_forward_LME(blocks)
...
% spend sweeping their trajectories in the forwards direction
...
frac_forward_premove(ind)=NaN; 
frac_forward_move(ind)=NaN; 

%% LME: trial-specific effects on fraction of time sweeping: forward premove

[fixed_forward_premove,random_forward_premove,R_vals_forward_premove,P_vals_forward_premove]=...
    linear_mixed_effects(blocks,frac_forward_premove,clrs.gold);
ylabel('Relative effect');

%% LME: trial-specific effects on fraction of time sweeping: forward move

[fixed_forward_move,random_forward_move,R_vals_forward_move,P_vals_forward_move]=...
    linear_mixed_effects(blocks,frac_forward_move,clrs.blue);
ylabel('Relative effect');
