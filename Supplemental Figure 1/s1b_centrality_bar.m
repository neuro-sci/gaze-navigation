function s1b_centrality_bar(blocks)
%% Produces a bar plot of arena centrality values
...
for arnum=1:5
    ...
    mean_centrality=mean(C); std_centrality=std(C,'omitnan');
    bar(arenas(arnum),mean_centrality,'FaceColor',clr(arnum,:),'EdgeColor','none')
    errorbar(arenas(arnum),mean_centrality,std_centrality,'color','k','CapSize',0)
end
...
ylabel('Centrality'); xlabel('Arena')
