function [PREMOVE,MOVE,sterr_premove,sterr_move]=interpolate_s12c(blocks,...
    arnum,n_subgoals,n_between,n_timepts_subgoal,n_timepts_between,k,u)

if nargin<5, u=0.5; if nargin<4, k=1.25; end; end
subjvec=[2,4,6,9,10,11,13,14,15,16,17,20,21];
n_timepts_premove=200; % interpoloate all pre-movement periods to 200 time points
PREMOVE=nan(700,n_timepts_premove); % pre-allocate a matrix larger than
% the total number of trials for an arena
PREMOVE_bySubject=nan(13,n_timepts_premove);
MOVE=nan(700,n_subgoals*n_timepts_subgoal+n_between*n_timepts_between);
